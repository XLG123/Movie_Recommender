...
class MovieDetailsViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
	...
	@IBAction func likeBtnPress(_ sender: UIButton) {
		if let _ = likeSelected   {
			...
			if likeSelected! {
				...
				if let movieIndex = likedMovieIds.firstIndex(of: Int64(movieSelected["id"] as! Int)) { // ChangeHere
	...
	func addUpdateCurrentMovie(isLike:Bool){
		...
		item.id = Int64(movieSelected["id"] as! Int) // ChangeHere
		item.isLike = isLike
	...
	func addToWatchList(){
		...
		let idVal = Int64(movieSelected["id"] as! Int) // ChangeHere
...
//		A notification for a successful attempt of adding movie to Watch List.
		let watchNotifySuccess = UIAlertController (title: "Added to Watch List", message: nil, preferredStyle: UIAlertController.Style.alert)
...
//		A notification for a successful attempt of moving movie from Watched to Watch List.
		let watchNotifySuccess2 = UIAlertController (title: "Moved to Watch List", message: nil, preferredStyle: UIAlertController.Style.alert)
		let cancelwatchNotify3 = UIAlertAction(title: "Done", style: UIAlertAction.Style.cancel, handler: nil)
				watchNotifySuccess2.addAction(cancelwatchNotify3)
 
//		A notification for an unsuccessful attempt.
//		Case 1: movie is already in Watch List.
		let watchNotifyFailure1 = UIAlertController (title: "Already in Watch List", message: nil, preferredStyle: UIAlertController.Style.alert)
...
//		Case 2: movie is in Watched List.
		...
		do {
			...
			if results1.count == 0{
				...
				if results2.count != 0{
					//remove item
					managedObjectContext.delete(results2[0])
					//setwatchNOtify --> moved item to watched list
					self.present(watchNotifySuccess2, animated: true, completion:nil)
				} else {
	...
	func addToWatchedList1(){
		...
		let managedObjectContext = appDel.persistentContainer.viewContext
		var results1: [WatchedItem] = [] //Have watched
		var results2: [WatchItem] = [] //Want to watch
...
//		A notification for a successful attempt of adding movie to Watched List.
		let watchedNotifySuccess = UIAlertController (title: "Added to Watched List", message: nil, preferredStyle: UIAlertController.Style.alert)
...
//		A notification for a successful attempt of moving movie from Watched to Watch List.
		let watchNotifySuccess2 = UIAlertController (title: "Moved to Watched List", message: nil, preferredStyle: UIAlertController.Style.alert)
		let cancelwatchNotify3 = UIAlertAction(title: "Done", style: UIAlertAction.Style.cancel, handler: nil)
				watchNotifySuccess2.addAction(cancelwatchNotify3)

//		A notification for an unsuccessful attempt.
//		Case 1: movie is already in Watched List.
		let watchedNotifyFailure1 = UIAlertController (title: "Already in Watched List", message: nil, preferredStyle: UIAlertController.Style.alert)
...
//		Case 3: movie is not released yet.
		...
		do {
			...
			if results1.count == 0 && release_date <= dateString{
			...
			} else { //if results1.count != 0 && results2.count == 0 (Already in list)
	...
	func addToWatchedList2(){
		...
		do {
			...
			if results1.count == 0 && results2.count == 0 && release_date <= dateString{
				...
				item.adult = movieSelected["adult"] as! Bool
				item.backdrop_path = movieSelected["backdrop_path"] as? String
				item.id = movieSelected["id"] as! Int64 // ChangeHere
				item.media_type = movieSelected["media_type"] as? String
				item.original_language = movieSelected["original_language"] as? String
				item.original_title = movieSelected["original_title"] as? String
				item.overview = movieSelected["overview"] as? String
